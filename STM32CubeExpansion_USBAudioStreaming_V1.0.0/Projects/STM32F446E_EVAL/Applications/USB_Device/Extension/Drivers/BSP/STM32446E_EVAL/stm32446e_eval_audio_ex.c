...
-----------------------
   ...
   + Call the function BSP_AUDIO_OUT_Init_Ext(
                                    OutputDevice: physical output mode (OUTPUT_DEVICE_SPEAKER, 
                                                  OUTPUT_DEVICE_HEADPHONE or OUTPUT_DEVICE_BOTH)
                                    Volume      : Initial volume to be set (0 is min (mute), 100 is max (100%)
                                    AudioFreq   : Audio frequency in Hz (8000, 16000, 22500, 32000...)
                                                  this parameter is relative to the audio file/stream type.
                                    Resolution  : audio samples resolution
                                   )
   ...
   + For each mode, you may need to implement the relative callback functions into your code.
      ...
      the stm32446e_eval_audio_ex.h file. (refer to the example for more details on the callbacks implementations)
   ...
   + The driver API and the callback functions are at the end of the stm32446e_eval_audio_ex.h file.
 

Driver architecture:
--------------------
   + This driver provides the High Audio Layer: consists of the function API exported in the stm32446e_eval_audio_ex.h file
...
uint32_t __IO AudioOutFreq = 48000; 
uint8_t __IO AudioOutResBit = 16; 
uint8_t __IO AudioOutResByte = 2;
...
/** @defgroup STM32446E_EVAL_AUDIO_Private_Function_Prototypes STM32446E EVAL AUDIO Private Function Prototypes
  * @{
  */
static void SAIx_Init_Ext(uint32_t AudioFreq, uint8_t AudioResolution);
static void SAIx_DeInit(void);
static void I2Sx_Init(uint32_t AudioFreq);
static void I2Sx_DeInit(void);
static void TIM_VolumeChangeIC_MspInit(TIM_HandleTypeDef *htim);
static void TIM_VolumeChangeIC_MspDeInit(TIM_HandleTypeDef *htim);
static void TIM_VolumeChangeInit(void);
static void TIM_VolumeChangeDeInit(void);
...
{ 
  ...
  SAIx_Init_Ext(AudioFreq,AudioResolution);
...
/**
  ...
  * @param  AudioFreq: Audio frequency used to play the audio stream.
  * @note   This API should be called after the BSP_AUDIO_OUT_Init_Ext() to adjust the
  *         audio frequency.
  */
...
void HAL_SAI_TxCpltCallback(SAI_HandleTypeDef *hsai)
{
  /* Manage the remaining file size and new address offset: This function 
     should be coded by user (its prototype is already declared in stm32446e_eval_audio_ex.h) */
...
{ 
  ...
  if(hsai->Instance == AUDIO_SAIx)
  {
    ...
    if(resolution == 24)
    {
      ...
      hdma_sai_tx.Init.PeriphDataAlignment = AUDIO_SAIx_DMAx_PERIPH_DATA_SIZE_24;
      hdma_sai_tx.Init.MemDataAlignment    = AUDIO_SAIx_DMAx_MEM_DATA_SIZE_24;
    }
    else
    {
      hdma_sai_tx.Init.PeriphDataAlignment = AUDIO_SAIx_DMAx_PERIPH_DATA_SIZE_16;
      hdma_sai_tx.Init.MemDataAlignment    = AUDIO_SAIx_DMAx_MEM_DATA_SIZE_16;
    }
...
/**
  ...
  * @param  AudioFreq: Audio frequency used to play the audio stream.
  * @param  Params: pointer on additional configuration parameters, can be NULL.
  * @note   This API is called by BSP_AUDIO_OUT_Init_Ext() and BSP_AUDIO_OUT_SetFrequency()
...
uint8_t BSP_AUDIO_IN_Init(uint32_t AudioFreq, uint32_t BitRes, uint32_t ChnlNbr)
{
  ...
  BSP_AUDIO_IN_ClockConfig(AudioFreq, 0);
...
uint8_t BSP_AUDIO_IN_Stop(void)
{
  ...
  /* TIM_VolumeChange Peripheral clock disable */
  AUDIO_TIM_VolumeChangeCLK_DISABLE();
...
uint8_t BSP_AUDIO_IN_PDMToPCM(uint16_t* PDMBuf, uint16_t* PCMBuf, uint8_t* temp_pdm, uint16_t pdm_buf_size )
{
  ...
  uint32_t index = 0; 
  /* PDM Demux */
  for(index = 0; index<pdm_buf_size/2; index++)
  {
    byte2 = (PDMBuf[index] >> 8)& 0xFF;
    byte1 = (PDMBuf[index] & 0xFF);
    temp_pdm[(index*2)+1] = Channel_Demux[byte1 & CHANNEL_DEMUX_MASK] | Channel_Demux[byte2 & CHANNEL_DEMUX_MASK] << 4;
    temp_pdm[(index*2)] = Channel_Demux[(byte1 >> 1) & CHANNEL_DEMUX_MASK] | Channel_Demux[(byte2 >> 1) & CHANNEL_DEMUX_MASK] << 4;
  }
  
  for(index = 0; index < DEFAULT_AUDIO_IN_CHANNEL_NBR; index++)
  {
    /* The PDM filter generates 16 samples of 16*bit from the 64 samples of the PDM format */
    PDM_Filter((uint8_t*)&temp_pdm[index], (uint16_t*)&(PCMBuf[index]), &PDM_FilterHandler[index]);
  }
  /* Return AUDIO_OK when all operations are correctly done */
  return AUDIO_OK; 
}
...
__weak void BSP_AUDIO_IN_MspInit(I2S_HandleTypeDef *hi2s, void *Params)
{
  ...
  /* Moved inside MSP to allow applic to redefine the TIM_VolumeChangeMspInit */
  TIM_VolumeChangeInit();
...
{ 
  ...
  if((AudioFreq == AUDIO_FREQUENCY_11K) || (AudioFreq == AUDIO_FREQUENCY_22K) || (AudioFreq == AUDIO_FREQUENCY_44K))
  {
    rcc_ex_clk_init_struct.PLLI2S.PLLI2SN = 220; 
    rcc_ex_clk_init_struct.PLLI2S.PLLI2SR = 20; 
  }
  else /* AUDIO_FREQUENCY_8K, AUDIO_FREQUENCY_16K, AUDIO_FREQUENCY_48K), AUDIO_FREQUENCY_96K */
  {
...
static void TIM_VolumeChangeIC_MspInit(TIM_HandleTypeDef *htim)
{
  ...
  AUDIO_TIM_VolumeChangeCLK_ENABLE();
    
  /* Enable GPIO Channels Clock */
  AUDIO_TIM_VolumeChangeGPIO_CLK_ENABLE();
  ...
  gpio_init_structure.Pull = GPIO_NOPULL;
  gpio_init_structure.Speed = GPIO_SPEED_HIGH;
  gpio_init_structure.Alternate = AUDIO_TIM_VolumeChangeAF;
  
  /* Configure TIM input channel */
  gpio_init_structure.Pin = AUDIO_TIM_VolumeChangeIN_GPIO_PIN;
  HAL_GPIO_Init(AUDIO_TIM_VolumeChangeGPIO_PORT, &gpio_init_structure);

  /* Configure TIM output channel */
  gpio_init_structure.Pin = AUDIO_TIM_VolumeChangeOUT_GPIO_PIN;
  HAL_GPIO_Init(AUDIO_TIM_VolumeChangeGPIO_PORT, &gpio_init_structure);
...
static void TIM_VolumeChangeIC_MspDeInit(TIM_HandleTypeDef *htim)
{
    /* Disable TIM_VolumeChange Peripheral clock  */
    AUDIO_TIM_VolumeChangeCLK_DISABLE();
...
/**
  ...
  *         I2S_SCK is externally connected to TIM_VolumeChange input channel
  */
static void TIM_VolumeChangeInit(void)
{
  ...
  haudio_tim.Instance = AUDIO_TIM_VolumeChange;
  /* Timer Input Capture Configuration Structure declaration */
   /* Initialize TIM_VolumeChange peripheral as follow:
       + Period = 0xFFFF
       + Prescaler = 0
       + ClockDivision = 0
  ...
  */
  ...
  /* Initialize the TIM_VolumeChange peripheral with the structure above */
  TIM_VolumeChangeIC_MspInit(&haudio_tim);
  ...
  HAL_TIM_IC_ConfigChannel(&haudio_tim, &s_ic_config, AUDIO_TIM_VolumeChangeIN_CHANNEL);
  ...
  HAL_TIM_PWM_ConfigChannel(&haudio_tim, &s_oc_config, AUDIO_TIM_VolumeChangeOUT_CHANNEL);
  
  /* Start the TIM4 Channel2 */
  HAL_TIM_PWM_Start(&haudio_tim, AUDIO_TIM_VolumeChangeOUT_CHANNEL);

  /* Start the TIM4 Channel1 */
  HAL_TIM_IC_Start(&haudio_tim, AUDIO_TIM_VolumeChangeIN_CHANNEL);
}
...
static void TIM_VolumeChangeDeInit(void)
{
  ...
  HAL_TIM_PWM_Stop(&haudio_tim, AUDIO_TIM_VolumeChangeOUT_CHANNEL);
  /* Stop the TIM4 Channel1 */
  HAL_TIM_IC_Stop(&haudio_tim, AUDIO_TIM_VolumeChangeIN_CHANNEL);

  HAL_TIM_IC_DeInit(&haudio_tim);
  
  /* Initialize the TIM_VolumeChange peripheral with the structure above */
  TIM_VolumeChangeIC_MspDeInit(&haudio_tim);
}
